export const APP_NAME = 'Lens Quick Widgets'
export const DEFAULT_THEME: 'light' | 'dark' = 'light'
export const EXAMPLE_POST_ID = '121frwetbr02new3gvk'
export const APP_LINK = 'https://lens-quick-widgets-web.vercel.app'
export const GITHUB_LINK = 'https://github.com/RathodDeven/lens-quick-widgets'
export const NPM_PACKAGE_LINK =
  'https://www.npmjs.com/package/lens-quick-widgets'
export const IPFS_ENDPOINT = 'https://4everland.io/ipfs/'
export const IMAGE_KIT_ENDPOINT = 'https://ik.imagekit.io/kopveel8c'
export const APP_ICON_LINK =
  'https://lens-quick-widgets-web.vercel.app/android-chrome-192x192.png'
